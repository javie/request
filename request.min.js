...
j=k;if(!b.isString(k)){j={key:l};}return this.config=b.defaults(j,this.config);};h.prototype.to=function(l,k,q){var j,m,p,n,o;if(q==null){q="json";}m=["POST","GET","PUT","DELETE"];
...
if(l==="?&"){l="";}this.execute=true;c.fire("Request.beforeSend",[this]);c.fire("Request.beforeSend: "+name,[this]);this.config.beforeSend(this);k={type:this.get("type"),dataType:this.get("dataType"),url:this.get("uri"),data:l,complete:function(n){var m;
...
n.put(l.config);m=n;}m=l;}else{m=new d;m.config=b.defaults(m.config,i.config);console.log(m.config);g[k]=m;}return m;};function i(j){return i.make(j);}i.make=function(j){if(!b.isString(j)){j="default";
